...
from instance_segmentation.lib.archs.reseg import ReSeg
from instance_segmentation.lib.archs.stacked_recurrent_hourglass import StackedRecurrentHourglass as SRecHg
from instance_segmentation.lib.losses.dice import DiceLoss, DiceCoefficient
from instance_segmentation.lib.losses.discriminative import DiscriminativeLoss
...
class Model(object):
    ...
    def __init__(self, dataset, model_name, n_classes, max_n_objects,
        ...
        assert self.dataset in ['CVPPP', 'HEBREW']
        assert self.model_name in ['ReSeg', 'StackedRecurrentHourglass']

        if self.dataset == 'CVPPP' or self.dataset == 'HEBREW':
    ...
    def __define_optimizer(self, learning_rate, weight_decay,
        ...
        parameters = filter(lambda p: p.requires_grad,
                             self.model.parameters())  # CHANGED from itertools ifilter to python3 filter
